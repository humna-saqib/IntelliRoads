...
class TraCISession:
    ...
    def _find_sumo_binary() -> str:
        ...
        Prefers the headless ``sumo`` binary since this is an unattended
        backend service — ``sumo-gui`` opens a window that (without
        ``--start``) waits for a manual play click and has no display to
        render to on a server, which hangs application startup. Set
        ``SUMO_USE_GUI=1`` to explicitly opt into the GUI binary instead.
        """
        ...
        # Fall back to whatever is on PATH, preferring the headless binary.
        return names[0]
